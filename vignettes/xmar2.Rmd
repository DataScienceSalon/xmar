...
As shown in `r kfigr::figr(label = "degreeBarPlot", prefix = TRUE, link = TRUE, type="Figure")` high school graduates comprise a near majority of the responses (`r round(max(uni$degree$data$p) * 100, 0)`%), followed by under graduates (`r round(uni$degree$data$p[4] * 100, 0)`%). Those with with less than high school degrees, those with graduate degrees, and respondents with junior college degrees make up `r round(uni$degree$data$p[1] * 100, 0)`%, `r round(uni$degree$data$p[5] * 100, 0)`%, , and `r round(uni$degree$data$p[3] * 100, 0)`%, respectively.
