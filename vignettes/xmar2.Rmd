...
```{r rq1, echo = F, message = FALSE, warning=FALSE}
rq1Data <- rq1(xmar)
```
...
### The Data
The segmented bar chart in `r kfigr::figr(label = "rq1Plot", prefix = TRUE, link = TRUE, type="Figure")` strongly suggests an association between opinions and behaviors with respect to extra-maritical conduct. Just `r rq1Data$data$propDf$Pct[2]`% of those with strong negative opinions reported extra-marital activity, versus `r rq1Data$data$propDf$Pct[4]`% of those with moderate opinions, and `r rq1Data$data$propDf$Pct[6]`% of those with relatively liberal positions on the matter.

```{r rq1Plot, include=T, echo = F, results='html'}
rq1Data$plots$propBar
```
`r kfigr::figr(label = "rq1Plot", prefix = TRUE, link = TRUE, type="Figure")`: Opinion vs Behavior Observed Frequencies and Proportions

`r kfigr::figr(label = "rq1ExpFreq", prefix = TRUE, link = TRUE, type="Table")` and `r kfigr::figr(label = "rq1ExpFreq", prefix = TRUE, link = TRUE, type="Table")` contain the observed and expected frequencies, under the null hypothesis. The chi-squared test of association will verify the statistical significance of the differences between the observed and expected frequencies. 

<div class = col2-left>
`r kfigr::figr(label = "rq1ExpFreq", prefix = TRUE, link = TRUE, type="Table")`: Opinion vs. Behavior Observed Frequency Table
```{r rq1ExpFreq, echo = F, results='html'}
knitr::kable(rq1Data$test$obsFreq, digits = 0, align = c("r", rep("c", 3))) %>%  
...
`r kfigr::figr(label = "rq1ExpProp", prefix = TRUE, link = TRUE, type="Table")`: Opinion vs. Behavior Expected Frequency Table 
```{r rq1ExpProp, echo = F, results='html'}
knitr::kable(rq1Data$test$expFreq, digits = 0, align = c("r", rep("c", 3))) %>%  
  kableExtra::add_header_above(c(" ", "Behavior" = 2, " ")) %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 
</div>
...
#### Inference Conditions
To evaluate the independence of opinions and behaviors, the **chi-square statistic** was selected.  The conditions necessary for performing chi-square tests of independence were evaluated as follows:  
* **Independence:** Each observation was subject to a stratified random sampling and so **independence** was assumed.   
* **Sample Size:** As shown in the `r kfigr::figr(label = "rq1ExpFreq", prefix = TRUE, link = TRUE, type="Table")`, the minimum threshold of five expected counts for each cell was met.  Furthermore, the sample size was less than 10% of the population of U.S. adults age 18 and over.
* **Degrees of Freedom:** The degrees of freedom, $df$, must be at least two.  In this case, there were indeed `r rq1Data$test$table$Df` degrees of freedom.  
Consequently, the conditions for performing a chi-square test of independence were met.  

#### Results and Interpretation
A chi-square test of independence was performed to examine the relation between opinion and behavior with respect to extra-marital conduct. `r kfigr::figr(label = "rq1X2Plot", prefix = TRUE, link = TRUE, type="Figure")` shows the chi-square distribution with the critical value, the reject region and observed statistic.  

```{r rq1X2Plot, include=T, echo = F, results='html'}
rq1Data$test$plot
```
`r kfigr::figr(label = "rq1X2Plot", prefix = TRUE, link = TRUE, type="Figure")`: Chi-Square Test of Association Plot

`r kfigr::figr(label = "rq1X2", prefix = TRUE, link = TRUE, type="Table")`: Opinion vs. Behavior Chi-Square Test of Association
```{r echo = F, results='html'}
knitr::kable(rq1Data$test$table) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 

As indicated in `r kfigr::figr(label = "rq1X2Plot", prefix = TRUE, link = TRUE, type="Figure")` and confirmed in `r kfigr::figr(label = "rq1X2", prefix = TRUE, link = TRUE, type="Table")`, the association between these variables was significant, X2 (`r rq1Data$test$table$Df`, $N$ = `r nrow(rq1Data$data$data)`) = `r round(rq1Data$test$table$X2_Observed, 0)`, $p$ = `r round(rq1Data$test$table$p_Value,3)`. **The data support an association between opinions and behaviors around extra-marital conduct.** 
