...
**Research Question 3:** Have opinions w.r.t. extra-marital conduct changed since the the beginning of 2000.  
* Dependent Variable: Opinion w.r.t. extra-marital conduct
* Grouping Variable: Years prior to and since 2000.
...
age <- analyze(data, y = "Opinion", x = "Age Group", success = "Traditional")
```
...
#### Data
The observed (`r kfigr::figr(label = "ageObservedPlot", prefix = TRUE, link = TRUE, type="Figure")`) and expected frequencies (`r kfigr::figr(label = "ageExpectedPlot", prefix = TRUE, link = TRUE, type="Figure")`) of opinion by age group indicated a trend towards more traditional views with increasing age. 

<div class = .col2-left>
```{r ageObservedPlot, echo = F, message = FALSE, warning=FALSE}
age$plots$observed
```
`r kfigr::figr(label = "ageObservedPlot", prefix = TRUE, link = TRUE, type="Figure")`: Observed Opinions by Age Group 
</div>

<div class = .col2-right>
```{r ageExpectedPlot, echo = F, message = FALSE, warning=FALSE}
age$plots$expected
```
`r kfigr::figr(label = "ageExpectedPlot", prefix = TRUE, link = TRUE, type="Figure")`: Expected Opinions by Age Group 
</div>


The following hypotheses characterize the direction and degree of association between opinion and age group.
...
where:   
$p1$ is the proportion of traditional opinion in age group 15-24  
$p2$ is the proportion of traditional opinion in age group 25-44  
$p3$ is the proportion of traditional opinion in age group 45-64  
$p4$ is the proportion of traditional opinion in age group 65+   
...
2. **Success/Failure:** There must be at least 10 successes and 10 failures for dichotomous categorical variables. Since the null hypotheses are equality conditions, the pooled proportions are used to calculate the numbers of expected successes and failures. As shown in `r kfigr::figr(label = "rq1ExpectedPlot", prefix = TRUE, link = TRUE, type="Figure")`, the number of traditional and traditional opinions far exceed this minimum threshold.   
...
* **Degrees of Freedom:** There were `r age$tests$independence$table$Df` degrees of freedom, greater than the minimum requirement of 2 degrees of freedom.  
...
##### Independence Test 
The hypotheses for the test of independence are restated as follows:  
$H_0$ Opinion and age group are independent. Therefore, no association exists between these two variables.   
$H_a$ Opinion and age group are **not** independent. Therefore, an association does exist between these two variables.   

`r age$tests$independence$stmt$type`


```{r rq1X2TestPlot, include=T, echo = F, results='html'}
age$tests$independence$table
```
`r kfigr::figr(label = "rq1X2TestPlot", prefix = TRUE, link = TRUE, type="Figure")`: Opinion vs age group independence test plot

As indicated in `r kfigr::figr(label = "rq1X2TestData", prefix = TRUE, link = TRUE, type="Table")`,  `r age$tests$independence$stmt$detail`
...
knitr::kable(age$tests$independence$table) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 

`r age$tests$independence$stmt$conclude`

##### Difference in Proportions Tests
To ascertain the direction and magnitude of the association, three sets of hypotheses were tested, one set to evaluate the differences in population proportion of Traditional opinion for each adjacent age group.  
...
$H_a$ $p1 - p2 \leq 0$    
where:  
$p1$ is the population proportion of traditional opinion in age group 15-24  
$p2$ is the population proportion of traditional opinion in age group 25-44  

`r age$tests$difference[[1]]$stmt$type` 

As indicated in `r kfigr::figr(label = "ageZTest1", prefix = TRUE, link = TRUE, type="Table")`,  `r age$tests$difference[[1]]$stmt$detail`

`r kfigr::figr(label = "ageZTest1", prefix = TRUE, link = TRUE, type="Table")`: Two-Proportion Z-Test of Opinion between age groups 15-24 and 25-44
```{r ageZTest1, echo = F, results='html'}
knitr::kable(age$tests$difference[[1]]$df) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 
where:  
* `r age$test$difference[[1]]$footnote[[1]]`  
* `r age$test$difference[[1]]$footnote[[2]]`  
* `r age$test$difference[[1]]$footnote[[3]]`  
* `r age$test$difference[[1]]$footnote[[4]]`  
* `r age$test$difference[[1]]$footnote[[5]]`  
* `r age$test$difference[[1]]$footnote[[6]]`  
* `r age$test$difference[[1]]$footnote[[7]]`   
* `r age$test$difference[[1]]$footnote[[8]]`   
* `r age$test$difference[[1]]$footnote[[9]]`   

`r  age$tests$difference[[1]]$stmt$conclude`  

###### Age Groups 25-44 and 45-64
The one-tailed hypothesis for second pair of proportions is restated as follows:  
$H_0$ $p1 - p2 = 0$   
$H_a$ $p1 - p2 \leq 0$    
where:  
$p1$ is the population proportion of traditional opinion in age group 25-44  
$p2$ is the population proportion of traditional opinion in age group 45-64  

`r age$tests$difference[[2]]$stmt$type` 

As indicated in `r kfigr::figr(label = "ageZTest2", prefix = TRUE, link = TRUE, type="Table")`,  `r age$tests$difference[[2]]$stmt$detail`

`r kfigr::figr(label = "ageZTest2", prefix = TRUE, link = TRUE, type="Table")`: Two-Proportion Z-Test of Opinion between age groups 25-44 and 45-64.
```{r ageZTest2, echo = F, results='html'}
knitr::kable(age$tests$difference[[2]]$df) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 
where:  
* `r age$test$difference[[2]]$footnote[[1]]`  
* `r age$test$difference[[2]]$footnote[[2]]`  
* `r age$test$difference[[2]]$footnote[[3]]`  
* `r age$test$difference[[2]]$footnote[[4]]`  
* `r age$test$difference[[2]]$footnote[[5]]`  
* `r age$test$difference[[2]]$footnote[[6]]`  
* `r age$test$difference[[2]]$footnote[[7]]`   
* `r age$test$difference[[2]]$footnote[[8]]`   
* `r age$test$difference[[2]]$footnote[[9]]`  

`r age$tests$difference[[2]]$stmt$conclude`

###### Age Groups 45-64 and 65+
The one-tailed hypothesis for third pair of proportions is restated as follows:  
$H_0$ $p1 - p2 = 0$   
$H_a$ $p1 - p2 \leq 0$    
where:  
$p1$ is the population proportion of traditional opinion in age group 45-64  
$p2$ is the population proportion of traditional opinion in age group 65+  

`r age$tests$difference[[3]]$stmt$type` 

As indicated in `r kfigr::figr(label = "ageZTest3", prefix = TRUE, link = TRUE, type="Table")`,  `r age$tests$difference[[3]]$stmt$detail`

`r kfigr::figr(label = "ageZTest3", prefix = TRUE, link = TRUE, type="Table")`: Two-Proportion Z-Test of Opinion between age groups 45-64 and 65+
```{r ageZTest3, echo = F, results='html'}
knitr::kable(age$tests$difference[[3]]$df) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 
where:  
* `r age$test$difference[[3]]$footnote[[1]]`  
* `r age$test$difference[[3]]$footnote[[2]]`  
* `r age$test$difference[[3]]$footnote[[3]]`  
* `r age$test$difference[[3]]$footnote[[4]]`  
* `r age$test$difference[[3]]$footnote[[5]]`  
* `r age$test$difference[[3]]$footnote[[6]]`  
* `r age$test$difference[[3]]$footnote[[7]]`   
* `r age$test$difference[[3]]$footnote[[8]]`   
* `r age$test$difference[[3]]$footnote[[9]]`

`r age$test$difference[[3]]$stmt$conclude`

#### Summary and Interpretation
Chi-square tests of independence were conducted to examine the association between opinion and age group. As summarized in `r kfigr::figr(label = "ageX2Summary", prefix = TRUE, link = TRUE, type="Table")`,  the association between variables was significant. $X2$ (`r age$test$independence$table$Df`, $N$ = `age$test$independence$table$N`) = `r round(age$test$independence$table$X2_Observed, 0)`, p-value = `r age$test$independence$table$p_Value`. Opinions with respect to extra-marital conduct and age group are associated.

`r kfigr::figr(label = "ageX2Summary", prefix = TRUE, link = TRUE, type="Table")`: Opinion vs age group independence test
```{r rq1X2Summary, echo = F, results='html'}
knitr::kable(age$tests$independence$table) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 

To ascertain the direction of the differences in proportions of opinion by age group in the population, two proportion tests were conducted for each pair of adjacent age groups and is summarized in `r kfigr::figr(label = "ageZSummary", prefix = TRUE, link = TRUE, type="Table")`. 

`r kfigr::figr(label = "ageZSummary", prefix = TRUE, link = TRUE, type="Table")`: Opinion vs age group difference in proportions
```{r rq1ZSummary, echo = F, results='html'}
df <- rbind(age$tests$difference[[1]]$df, age$tests$difference[[2]]$df, age$tests$difference[[3]]$df)
knitr::kable(df) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 
`r kfigr::figr(label = "ageZSummary", prefix = TRUE, link = TRUE, type="Table")` shows that the differences in proportions of Traditional opinion in the population are significant between age groups 18-24 and and the other age groups; however, the differences in proportions of Traditional opinion were not significant among age groups 25 and older.


### Research Question 2: Opinions and Gender
```{r genderAnalysis, echo = F, message = FALSE, warning=FALSE}
data <- xmar %>% select(Opinion, Gender)
gender <- analyze(data, y = "Opinion", x = "Gender", success = "Traditional")
```
...
#### The Data
The observed (`r kfigr::figr(label = "genderObservedPlot", prefix = TRUE, link = TRUE, type="Figure")`) and expected frequencies (`r kfigr::figr(label = "genderExpectedPlot", prefix = TRUE, link = TRUE, type="Figure")`) of opinion by gender indicate that the proportion of the female population that holds Traditional views w.r.t. extra-marital conduct exceeds the proportion of the male population that holds such views.

<div class = .col2-left>
```{r genderObservedPlot, echo = F, message = FALSE, warning=FALSE}
gender$plots$observed
```
`r kfigr::figr(label = "ageObservedPlot", prefix = TRUE, link = TRUE, type="Figure")`: Observed Opinions by Age Group 
</div>

<div class = .col2-right>
```{r genderExpectedPlot, echo = F, message = FALSE, warning=FALSE}
gender$plots$expected
```
`r kfigr::figr(label = "ageExpectedPlot", prefix = TRUE, link = TRUE, type="Figure")`: Expected Opinions by Age Group 
</div>
...
#### Hypotheses
The differences in proportion of traditional opinion by gender was evaluated via the following hypotheses:

$H_0$ $p1 = p2$  
$H_a$ $p1 \neq p2$  

where:   
$p1$ is the proportion of traditional opinion among males in the population
$p2$ is the proportion of traditional opinion among females in the population
...
1. **Independence:** Each observation was subject to a stratified random sampling and so **independence** was assumed.   
2. **Success/Failure:** There must be at least 10 successes and 10 failures for dichotomous categorical variables.  As shown in `r kfigr::figr(label = "ageExpectedPlot", prefix = TRUE, link = TRUE, type="Figure")`, the numbers from the sample holding  traditional and non-traditional views exceed the minimum threshold of 10.
3. **Sample Size:** As indicated in section the methods section, the sample size falls well below 10% of the population.  

As such, the conditions for two-proportion z-tests were met.

#### Test results 
`r gender$tests$difference$stmt$type` 

As indicated in `r kfigr::figr(label = "genderZTest", prefix = TRUE, link = TRUE, type="Table")`,  `r gender$tests$difference$stmt$detail`

`r kfigr::figr(label = "genderZTest", prefix = TRUE, link = TRUE, type="Table")`: Two-proportion z-test of opinion by gender
```{r genderZTest, echo = F, results='html'}
knitr::kable(gender$tests$difference$df) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 
where:  
* `r gender$tests$difference$footnote[[1]]`  
* `r gender$tests$difference$footnote[[2]]`  
* `r gender$tests$difference$footnote[[3]]`  
* `r gender$tests$difference$footnote[[4]]`  
* `r gender$tests$difference$footnote[[5]]`  
* `r gender$tests$difference$footnote[[6]]`  
* `r gender$tests$difference$footnote[[7]]`  
* `r gender$tests$difference$footnote[[8]]`  
* `r gender$tests$difference$footnote[[9]]`  
* `r gender$tests$difference$footnote[[10]]`  
* `r gender$tests$difference$footnote[[11]]`  
* `r gender$tests$difference$footnote[[12]]`  
* `r gender$tests$difference$footnote[[13]]`  

`r gender$tests$difference$stmt$conclude`

### Research Question 3: Evolution of Opinion
The evolution of opinion w.r.t. extra-marital conduct was examined via the following question.
> Have opinions w.r.t. extra-marital conduct changed since the the beginning of 2000.  

```{r yearsAnalysis, echo = F, message = FALSE, warning=FALSE}
data <- xmar %>% select(Opinion, Years)
years <- analyze(data, y = "Opinion", x = "Millennium", success = "Traditional")
```

#### The Data
The observed (`r kfigr::figr(label = "yearsObservedPlot", prefix = TRUE, link = TRUE, type="Figure")`) and expected frequencies (`r kfigr::figr(label = "yearsExpectedPlot", prefix = TRUE, link = TRUE, type="Figure")`) of opinion by millennium indicate that the proportion of the population holding traditional views have increased since the year 2000.

<div class = .col2-left>
```{r yearsObservedPlot, echo = F, message = FALSE, warning=FALSE}
years$plots$observed
```
`r kfigr::figr(label = "ageObservedPlot", prefix = TRUE, link = TRUE, type="Figure")`: Observed Opinions by Millennium 
</div>

<div class = .col2-right>
```{r yearsExpectedPlot, echo = F, message = FALSE, warning=FALSE}
years$plots$expected
```
`r kfigr::figr(label = "ageExpectedPlot", prefix = TRUE, link = TRUE, type="Figure")`: Expected Opinions by Millennium 
</div>
...
where:   
$p1$ is the proportion of traditional opinion prior to the year 2000.
$p2$ is the proportion of traditional opinion since the year 2000.

#### Inference Conditions
The inference conditions for a two-proportion test for differences in the proportion of traditional opinion, were examined as follows:
...
#### Test results 
`r years$tests$difference$stmt$type` 

As indicated in `r kfigr::figr(label = "yearsZTest", prefix = TRUE, link = TRUE, type="Table")`,  `r years$tests$difference$stmt$detail`

`r kfigr::figr(label = "yearsZTest", prefix = TRUE, link = TRUE, type="Table")`: Two-proportion z-test of opinion by millennium
```{r yearsZTest, echo = F, results='html'}
knitr::kable(years$tests$difference$df) %>%  
  kableExtra::kable_styling(bootstrap_options = c("striped", "condensed"), full_width = T, position = "center")
``` 
where:  
* `r years$tests$difference$footnote[[1]]`  
* `r years$tests$difference$footnote[[2]]`  
* `r years$tests$difference$footnote[[3]]`  
* `r years$tests$difference$footnote[[4]]`  
* `r years$tests$difference$footnote[[5]]`  
* `r years$tests$difference$footnote[[6]]`  
* `r years$tests$difference$footnote[[7]]`  
* `r years$tests$difference$footnote[[8]]`  
* `r years$tests$difference$footnote[[9]]`  
* `r years$tests$difference$footnote[[10]]`  
* `r years$tests$difference$footnote[[11]]`  
* `r years$tests$difference$footnote[[12]]`  
* `r years$tests$difference$footnote[[13]]`  

`r years$tests$difference$stmt$conclude`
