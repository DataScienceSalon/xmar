...
# Abstract
# Introduction
Researchers from the University of Michigan, Indiana University and Rutgers University conducted a study of the prevalence of consensual nonmonogamy (CNM) is increasing, little is known about the prevalence of CNM. Using two separate U.S. Census based quota samples of single adults in the United States (Study 1: n = 3,905; Study 2: n = 4,813), the present studies show that more than one in five (21.9% in Study 1; 21.2% in Study 2) According to a study published in the Journal of Sex & Marital Therapy, [@doi:10.1080/0092623X.2016.1178675], 21% 
...
This exploratory data analysis revealed the frequencies and proportions of responses by the Period, Age Group, Gender, Male Age Group, and Female Age Group variables.  Since the independence and sample size requirements were met, the inferences with respect to the population parameters, are valid.   
...
data1 <- xmar %>% filter(AgeGroup == "15-24") %>% select(Opinion, Period) 
data2 <- xmar %>% filter(AgeGroup == "25-44") %>% select(Opinion, Period) 
data3 <- xmar %>% filter(AgeGroup == "45-64") %>% select(Opinion, Period) 
data4 <- xmar %>% filter(AgeGroup == "65+") %>% select(Opinion, Period) 
...
data1 <- xmar %>% filter(Gender == "Male") %>% select(Opinion, Period) 
data2 <- xmar %>% filter(Gender == "Female") %>% select(Opinion, Period) 
...
data1 <- xmar %>% filter(GenderAge == "Male 15-24") %>% select(Opinion, Period) 
data2 <- xmar %>% filter(GenderAge == "Male 25-44") %>% select(Opinion, Period) 
data3 <- xmar %>% filter(GenderAge == "Male 45-64") %>% select(Opinion, Period) 
data4 <- xmar %>% filter(GenderAge == "Male 65+") %>% select(Opinion, Period) 
data5 <- xmar %>% filter(GenderAge == "Female 15-24") %>% select(Opinion, Period) 
data6 <- xmar %>% filter(GenderAge == "Female 25-44") %>% select(Opinion, Period) 
data7 <- xmar %>% filter(GenderAge == "Female 45-64") %>% select(Opinion, Period) 
data8 <- xmar %>% filter(GenderAge == "Female 65+") %>% select(Opinion, Period) 
